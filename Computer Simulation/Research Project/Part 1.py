...
if __name__ == "__main__":
    ...
    TickerNWeights = pd.read_excel('VNQ.xlsx', sheet_name='Sheet1', header=0, index_col=0)
    Ticker_AllStock_DJ = TickerNWeights['Symbol']
    wts_AllStock_DJ = TickerNWeights['Weight']
    #Price_AllStock_DJ = TickerNWeights['Price']
    
    #%% get historical stock price data
    Flag_downloadData = True
    # define the time period 
    start_dt = datetime.datetime(2010, 3, 19)
    end_dt = datetime.datetime(2019, 12, 31)
    
    if Flag_downloadData:
        DJData = pdr.get_data_yahoo('VNQ', start=start_dt, end=end_dt)
        #
    ...
    plt.title('VNQ 30 ETF', fontsize=18)
    ax.xaxis.set_tick_params(labelsize=14)
    ax.yaxis.set_tick_params(labelsize=14)
        #%%
    # plot the tracking error daily
    # define the time period 
    dates1 = DJData.index
    TE_daily1 = np.zeros(len(dates1))
    
    
    for i in range(len(dates1)-1):
     
        cov_daily = var_ewma[i,:]

        TE_daily1[i] = tracking_error(wts_active2,cov_daily)
        
        
    # plot daily tracking error
    plt.plot(dates1, TE_daily1)
    plt.title('Daily Tracking Error')
    plt.xlabel('Date')
    ...
    end_dt = datetime.datetime(2022, 12, 31)
    ...
    # calc TE achieved
    wts_active2 = wts_min_trackingerror2 - wts_AllStock_DJ
    TE_optimized3 = tracking_error(wts_active2,cov_end_annual)
    print('{0} top weighted stock replication TE = {1:5.2f} bps'.format(num_topwtstock_2include, TE_optimized3*10000))
    
            #%%
    # plot the tracking error daily
    # define the time period 
    dates2 = DJData.index
    TE_daily2 = np.zeros(len(dates2))
    ...
    for i in range(len(dates2)-1):
        ...
        TE_daily2[i] = tracking_error(wts_active2,cov_daily)
        
        
    # plot daily tracking error
    plt.plot(dates2, TE_daily2)
    plt.title('Daily Tracking Error')
    plt.xlabel('Date')
    plt.ylabel('Tracking Error')
    plt.show()
